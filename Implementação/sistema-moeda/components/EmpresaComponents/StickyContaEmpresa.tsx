...
const StickyContaEmpresa = ({ data }: Props) => {
  const { id, nome, cnpj, empresa } = data;
  ...
  const handleSendMoney = async () => {
    ...
    const { data } = await axios.patch(`/api/users/empresaes/${empresaId}/enviarMoeda`, {
      valor: value, empresaId: id
    });
  ...
  return (
    <div
      ...
      <div className="">
        ...
        <p className="text-gray-800 text-sm">CPF: {cnpj}</p>
      </div>
      <div>
        <div className="flex">
...
export default StickyContaEmpresa;